#!/bin/vbash
set interfaces ethernet eth0 description 'WAN - Cable'
set interfaces ethernet eth0 address 'dhcp'

set interfaces ethernet eth1 vif 900 address '10.10.90.1/24'
set interfaces ethernet eth1 vif 900 description 'unifi-mgmt-900'
set interfaces ethernet eth1 vif 910 address '10.10.91.1/24'
set interfaces ethernet eth1 vif 910 description 'unifi-frontend-910'
set interfaces ethernet eth1 vif 120 address '10.10.120.1/24'
set interfaces ethernet eth1 vif 120 description 'k8s-120'
set interfaces ethernet eth1 vif 550 address '10.10.55.1/24'
set interfaces ethernet eth1 vif 550 description 'iLO-550'
set interfaces ethernet eth1 vif 11 address '10.10.11.1/24'
set interfaces ethernet eth1 vif 11 description 'pve-11'
set interfaces ethernet eth1 vif 610 address '10.10.60.1/24'
set interfaces ethernet eth1 vif 610 description 'seccam-610'
set interfaces ethernet eth1 vif 140 address '10.10.140.1/24'
set interfaces ethernet eth1 vif 140 description 'UISP-140'
set interfaces ethernet eth1 vif 10 address '172.16.1.250/24'
set interfaces ethernet eth1 vif 10 description 'transit-10'
set interfaces ethernet eth1 vif 110 address '10.10.10.1/24'
set interfaces ethernet eth1 vif 110 description 'ad-110'
set interfaces ethernet eth1 vif 410 address '10.10.40.1/24'
set interfaces ethernet eth1 vif 410 description 'bastion-410'
set interfaces ethernet eth1 vif 720 address '10.10.72.1/24'
set interfaces ethernet eth1 vif 720 description 'app-720'

set interfaces wireguard wg01 address '10.10.30.1/24'
set interfaces wireguard wg01 description 'WIREGUARD'
set interfaces wireguard wg01 peer it-pc01 allowed-ips '10.10.30.10/32'
set interfaces wireguard wg01 peer it-pc01 persistent-keepalive '15'
set interfaces wireguard wg01 peer it-pc01 public-key 'RLV0A32MXIFkqLuwLdgpJVDaerxLcCexUCaGOkNte3w='
set interfaces wireguard wg01 peer macbook-it allowed-ips '10.10.30.11/32'
set interfaces wireguard wg01 peer macbook-it persistent-keepalive '15'
set interfaces wireguard wg01 peer macbook-it public-key 'cT+qa6B3XMAVENQ2EOK2nfpnXZUGp5RkUwkUSEQZWVc='
set interfaces wireguard wg01 peer xps-it allowed-ips '10.10.30.12/32'
set interfaces wireguard wg01 peer xps-it persistent-keepalive '15'
set interfaces wireguard wg01 peer xps-it public-key 'ftYscBzFt6+7zBotvBR8MO9M1PhziWtg+s9FKJmZskc='
set interfaces wireguard wg01 port '51820'
set interfaces wireguard wg01 private-key "${SECRET_WIREGUARD_PRIVATE_KEY}"

set interfaces wireguard wg02 address '10.10.31.1/24'
set interfaces wireguard wg02 description 'WIREGUARD-AO'
set interfaces wireguard wg02 peer it-mobile allowed-ips '10.10.31.10/32'
set interfaces wireguard wg02 peer it-mobile persistent-keepalive '15'
set interfaces wireguard wg02 peer it-mobile public-key 'K910+sQUVYHfbUfoN0HKlIObkqBY0P4Efu+kYw97Ylc='
set interfaces wireguard wg02 peer mh-mobile allowed-ips '10.10.31.11/32'
set interfaces wireguard wg02 peer mh-mobile persistent-keepalive '15'
set interfaces wireguard wg02 peer mh-mobile public-key 'E8FPyFCG/gAr8XFSlQ8nhlJdikGX53yjQERiUnLifTY='
set interfaces wireguard wg02 port '51821'
set interfaces wireguard wg02 private-key "${SECRET_WIREGUARD_PRIVATE_KEY}"
#############

# set interfaces ethernet eth0 description 'WAN - er01'
# set interfaces ethernet eth0 hw-id 'e4:3a:6e:5f:33:9c'
# set interfaces ethernet eth0 mtu '1508'
# set interfaces ethernet eth0 vif 6 description 'Internet'
# set interfaces ethernet eth0 vif 6 mtu '1508'

# set interfaces ethernet eth1 address '10.1.0.1/24'
# set interfaces ethernet eth1 description 'LAN'
# set interfaces ethernet eth1 hw-id 'e4:3a:6e:5f:33:9d'
# set interfaces ethernet eth1 vif 10 address '10.1.1.1/24'
# set interfaces ethernet eth1 vif 10 description 'SERVERS'
# set interfaces ethernet eth1 vif 20 address '10.1.2.1/24'
# set interfaces ethernet eth1 vif 20 description 'TRUSTED'
# set interfaces ethernet eth1 vif 30 address '192.168.2.1/24'
# set interfaces ethernet eth1 vif 30 description 'GUEST'
# set interfaces ethernet eth1 vif 40 address '10.1.3.1/24'
# set interfaces ethernet eth1 vif 40 description 'IOT'
# set interfaces ethernet eth1 vif 50 address '10.1.4.1/24'
# set interfaces ethernet eth1 vif 50 description 'VIDEO'

# set interfaces ethernet eth2 hw-id 'e4:3a:6e:5f:33:9e'
# set interfaces ethernet eth3 hw-id 'e4:3a:6e:5f:33:9f'
# set interfaces ethernet eth4 hw-id 'e4:3a:6e:5f:33:a0'

# set interfaces ethernet eth5 hw-id 'e4:3a:6e:5f:33:a1'
# set interfaces ethernet eth5 description 'WAN - Cable'
# set interfaces ethernet eth5 address 'dhcp'

# set interfaces pppoe pppoe0 description 'WAN - Fiber'
# set interfaces pppoe pppoe0 source-interface 'eth0.6'
# set interfaces pppoe pppoe0 mtu '1500'
# set interfaces pppoe pppoe0 authentication user "${SECRET_ISP_AUTH_USER}"
# set interfaces pppoe pppoe0 authentication password "${SECRET_ISP_AUTH_PASSWORD}"

# set interfaces wireguard wg01 address '10.0.11.1/24'
# set interfaces wireguard wg01 description 'WIREGUARD'
# set interfaces wireguard wg01 peer ipad-bernd allowed-ips '10.0.11.4/32'
# set interfaces wireguard wg01 peer ipad-bernd persistent-keepalive '15'
# set interfaces wireguard wg01 peer ipad-bernd public-key 'EYuUoDoCIcSYbnjNyPSsZUcz4Yp032AHWflYlH2V1EM='
# set interfaces wireguard wg01 peer iphone-bernd allowed-ips '10.0.11.2/32'
# set interfaces wireguard wg01 peer iphone-bernd persistent-keepalive '15'
# set interfaces wireguard wg01 peer iphone-bernd public-key '608IT2YRPTCabLMGnpYfIWhgDnEpLV5IIpYi5J7m3As='
# set interfaces wireguard wg01 peer macbook-bernd allowed-ips '10.0.11.3/32'
# set interfaces wireguard wg01 peer macbook-bernd persistent-keepalive '15'
# set interfaces wireguard wg01 peer macbook-bernd public-key 'iWKTc88fDBhqlsAgoCXYvkBdf9wG4YBZ8XTg/+0Ln2I='
# set interfaces wireguard wg01 port '51820'
# set interfaces wireguard wg01 private-key "${SECRET_WIREGUARD_PRIVATE_KEY}"
